...
/**
 * Main class for the default course format
 *
 * @package    format_default
 * @copyright  2012 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_default extends format_base {
    ...
    /**
     * Returns the default section name for the default course format.
     *
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_default');
        } else {
     ...
     *
     * default format uses the following options:
     * - coursedisplay
     * - hiddensections
     *
     ...
     *
     * In case if course format was changed to 'default', we try to copy options
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_default');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_default', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'default' allows to set and remove markers in addition to common section actions.
        ...
        $renderer = $PAGE->get_renderer('format_default');
 ...
 * @return \core\output\inplace_editable
 */
function format_default_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'default'), MUST_EXIST);
